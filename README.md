...
### ✅ Integration Test 케이스
| 테스트 ID | 기능                    | 테스트 내용                     | 시나리오                                    | 기대 결과                                              |
| ------ | --------------------- | -------------------------- | --------------------------------------- | -------------------------------------------------- |
| IT-001 | 회원가입 후 로그인까지 전체 플로우   | 회원가입 → 로그인 → 토큰 저장 확인      | 회원가입 폼 입력 → 제출 → 로그인 페이지 이동 → 로그인       | DB에 사용자 생성, 토큰 생성 및 localStorage/sessionStorage 저장 |
...
| IT-011 | 카카오페이 강의 환불  | 프론트에서 환불 요청 → 백엔드 환불 처리 → 카카오페이 응답 → DB 반영 | 환불 버튼 클릭 → 확인 → 환불 요청 전송 → 카카오페이 응답 → 수강권 제거 | 프론트, DB 상태 'CANCELED'로 변경, 수강 권한 제거 |
